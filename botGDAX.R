...
if(curr_bal_usd() >= 20){    # if have more than $20 USD start loop
  if(curr_rsi14_api >= 30 &    # and current rsi >= 35  # v.2
    ...
    Sys.sleep(360) #reduced frequency on overload
    ...
    send.mail(from = "jasonczmeng@gmail.com",
              to = c("jasonczmeng@gmail.com"),
              replyTo = c("Reply to someone else <jasonczmeng@gmail.com>"),
    ...
    # Numbers changed for new strategy
    # Order 1: take 1/3 profits at 5% gain #increased from 1 to counter slippage
    order_size_tiered3 <- round(curr_bal_eth()/3,3)
    order_price_tiered3 <- round(bid() * 1.05,2)
    Sys.sleep(17)
    add_order(product_id = "ETH-USD", api.key = "your_api_key", secret = "your_secret", passphrase = "your_passphrase",
              type="limit", price = order_price_tiered3, side = "s",  size = order_size_tiered3 )
  
    Sys.sleep(17)
    # Order 2: take 1/3 profits at 10% gain
    order_size_tiered8 <- round(curr_bal_eth(),3)
    order_price_tiered8 <- round(bid() * 1.10,2)
    add_order(product_id = "ETH-USD", api.key = "your_api_key", secret = "your_secret", passphrase = "your_passphrase",
              type="limit", price = order_price_tiered8, side = "s",  size = order_size_tiered8 )
    
    Sys.sleep(17)
    # Order 3: take 1/3 profits at 15% gain
    order_size_tiered5 <- round(curr_bal_eth()/2,3)
    order_price_tiered5 <- round(bid() * 1.15,2)
    add_order(product_id = "ETH-USD", api.key = "your_api_key", secret = "your_secret", passphrase = "your_passphrase",
